 ...
 * 否则夏令时切换后取到的本地时间会一直偏差(ZLToolKit内部由时间戳线程按刻钟边界触发)。该函数内部调用localtime_r()/localtime_s()，会加锁且不是fork安全的，
 * 故调用方必须避开热点路径并控制调用频次。
 ...
 * local time would stay off after a switch (inside ZLToolKit the timestamp thread
 * triggers it on quarter hour boundaries). This function calls localtime_r() or
